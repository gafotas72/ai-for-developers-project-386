...
public interface SlotRepository extends JpaRepository<Slot, Long> {
     ...
     */
    @Query(value = "SELECT s.* FROM slots s INNER JOIN event_types e ON s.event_type_id = e.id WHERE e.user_id = :ownerId AND CAST(s.start_time AS DATE) >= CAST(:startDate AS DATE) AND s.start_time > :startDate", nativeQuery = true)
